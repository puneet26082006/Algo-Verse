...
string helper(ll k , string &s , ll n){
    vector<pair<pair<ll, ll>, ll>> hashes ;
    hashes.reserve(n - k + 1);

    for(ll i = 0 ; i <= n - k ; i++){
        hashes.pb({{get_hash(i, i + k - 1)}, i});
    }

    sort(hashes.begin(), hashes.end());

    for(ll i = 1 ; i < (ll)hashes.size() ; i++){
        if(hashes[i].ff == hashes[i - 1].ff){
            return s.substr(hashes[i].ss, k) ;
        }
    }
