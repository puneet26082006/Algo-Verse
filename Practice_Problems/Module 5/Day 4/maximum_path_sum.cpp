...
int helper(vector<vector<int>>& mat,int N, int M, int row, int col){
    ...
    if((row < N - 1) && (col < M - 1)){
        return max(helper(mat, N, M, row + 1, col), helper(mat, N, M, row, col + 1)) + mat[row][col];
    } else if(row == N - 1){
        return helper(mat, N, M, row, col + 1) + mat[row][col];
    } else {
        return helper(mat, N, M, row + 1, col) + mat[row][col];
    }
