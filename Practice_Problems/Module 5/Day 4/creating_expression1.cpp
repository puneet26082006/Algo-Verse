...
bool helper(vector<int> & a, int n , int x, int value, int idx ){
    if(idx == n){
        return value == x ;
    }

    int ans1 = helper(a, n, x, (value - a[idx]), idx + 1);
    int ans2 = helper(a, n, x, (value + a[idx]), idx + 1);
