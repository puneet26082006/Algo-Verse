...
int helper(vector<int>& v, vector<int>& w, int N, int W, int idx){
    if(idx >= N){
        return 0 ;
    }

    if(W >= w[idx]){
        return max(helper(v,w,N,W,idx + 1), helper(v,w,N, (W - w[idx]), idx + 1) + v[idx]);
    }else {
        return helper(v,w,N,W,idx + 1);
    }
}
