...
bool helper(int mid, vector<pair<int, int>>& segments){
    pair<int, int> curr = {0,0};

    for(int i = 0 ; i < segments.size() ; i++){
        pair<int, int> jump = {curr.ff - mid, curr.ss + mid} ;
        pair<int, int> seg = segments[i];

        curr = {max(seg.ff, jump.ff), min(seg.ss, jump.ss)};
