...
void precompute() {
    ...
    ifact[N - 1] = expo(fact[N - 1], mod - 2 , mod);
...
void solve() {
    ...
    for(ll i = 2 ; i < n ; i+= 3){
        ll count = 0 ;
        ll max_weight = max({a[i] + a[i - 1], a[i] + a[i - 2], a[i - 1] + a[i - 2]});
        if(a[i] + a[i - 1] == max_weight) count++ ;
        if(a[i] + a[i - 2] == max_weight) count++ ;
        if(a[i - 1] + a[i - 2] == max_weight) count++ ;

        ans = (ans * count) % mod ;

    }

    cout<< (choose(n/3 , n/6) * ans ) % mod <<endl ;
