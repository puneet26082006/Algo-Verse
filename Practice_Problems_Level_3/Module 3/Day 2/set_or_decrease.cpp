...
bool helper(ll steps, vector<ll>& a, ll n , ll k){
    ll max_steps = min(steps, n - 1);
    ll total_sum = accumulate(a.begin(), a.end(), 0LL);

    for(ll i = 0 ; i <= max_steps ; i++){
        if(i == 0){
            ll ele = a[0] - steps ;
            if(total_sum - a[0] + ele <= k){
                return true ;
            }
        } else {
            ll right = i ;
            ...
            total_sum = total_sum - a[n - right];

            ll ele_left = a[0] - left ;

            ll new_sum = total_sum - a[0] + (right + 1) * ele_left ;
