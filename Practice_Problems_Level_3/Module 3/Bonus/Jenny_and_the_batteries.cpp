...
bool helper(ll x , ll k , vector<ll> & a, vector<ll> & b, vector<ll> & c){
    ll space = 0 ;
    ll items = 0 ;
    ll total_cost = 0 ;
    ...
    ll items_check = items ;
    for(ll i = 0 ; i < rec.size() && items_check > 0 ; i++){
        ll take = min(items_check, rec[i].ss);
        total_cost += rec[i].ff * take ;
        items_check -= take ;
...
void solve() {
    ...
    ll si = 0 ;
    ll ei = *max_element(a.begin(), a.end()) ;
    ll ans = -1 ;

    while(si <= ei){
        ll mid = (si + ei)/ 2 ;

        if(helper(mid, k, a, b, c)){
            ans = mid ;
            ei = mid - 1 ;
        } else {
            si = mid + 1 ;
