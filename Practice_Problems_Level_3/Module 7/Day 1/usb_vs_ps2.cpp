...
void solve() {
    ...
    ll cnt = 0 ;
    ll cost = 0 ;
    for(ll i = 0 ; i < a && i < usb.size() ; i++){
        cost += usb[i] ;
        cnt += 1 ;

        both.erase(both.find(usb[i]));
    }

    for(ll i = 0 ; i < b && i < ps2.size() ; i++){
        cost += ps2[i];
        cnt += 1 ;

        both.erase(both.find(ps2[i]));
    }

    for(ll i = 0 ; i < c && !both.empty() ; i++){
        cost += *both.begin() ;
        cnt += 1 ;

        both.erase(both.find(*both.begin()));
    }
